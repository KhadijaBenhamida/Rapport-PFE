...
As well as the operations by the user, this same service is reused for the event driven governance communication:

\begin{itemize}
    \item[\quad\textbullet] responsible assignment update confirmation,
    \item[\quad\textbullet] notification of workload,
    \item[\quad\textbullet] gate progression alert (planned extension).
\end{itemize}

\noindent The automated messages take their wording from the approved templates available in current context variables (project, component, actor, dates) that are automatically inserted, so everything is regular and matches the rules.
...
\begin{tabularx}{0.95\textwidth}{|l|X|}
    ...
    \hline
    \texttt{USER\_NOT\_FOUND} & Recipient query does not match any directory entry. Copilot asks for recipient refinement. \\
    \hline
    \texttt{AMBIGUOUS\_USER} & Multiple users match the query. Copilot requests explicit selection. \\
    \hline
    \texttt{MISSING\_SUBJECT} & Subject line is missing. Copilot asks for the subject. \\
    \hline
    \texttt{MISSING\_BODY} & Message body is missing. Copilot asks for message content. \\
    \hline
    \texttt{INVALID\_EMAIL} & Resolved email address is invalid. System stops and asks for correction. \\
    \hline
    \texttt{OUTLOOK\_UNAVAILABLE} & Outlook connector or mail service is unavailable. Attempt is logged and user is advised to retry. \\
    \hline
    \texttt{SEND\_FAILURE} & Transmission fails (policy, quota, or network issue). Error details are returned with escalation guidance. \\
    \hline
...
\begin{itemize}
    \item[\quad\textbullet] \textbf{Draft mode}: confirmation that an editable Outlook draft was created.
    \item[\quad\textbullet] \textbf{Send success}: conversational confirmation with recipient, subject, and timestamp.
    \item[\quad\textbullet] \textbf{Send failure}: the error code, a short description and advice on what to do.
    \item[\quad\textbullet] \textbf{Automated notifications}: never prompt the user, full state is always maintained within flow history and audit logs.
\end{itemize}
